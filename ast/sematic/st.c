...
/*
generate a struct symbol and return its address if succeed, else return NULL
para: structNode-> STRUCT_TYPE node
*/

Symbol *genStructSym(Node *structNode)
{    
    //int errorOccurred = 0;
    int count = 0;
    char *name = structNode->children[0]->val;
    Node* defList;
    ...
    strncpy(structSym->name, name, strlen(name));
    ...
    for (int i = 0; i < defList->num; ++i)
    {
        ...
        if(type.type == ERROR)
        {
            // use struct name to define member variable but the struct hasn't bee defined
            ...
            else
            {
                ...
                // struct domain name duplicated
                if(sym)
                    printf("Error type %d at Line %d: symbol \"%s\" was declared before.\n",
                        STRUCT_NAME_REPEATED, idNode->lineNo);
                else
                {
                    sym = genStructSym(def->children[0]);
                    type.type = STRUCT;
                    type.symAddr = sym;
                    goto L;
                }                    
...
{    
    Symbol* structSym = genStructSym(structNode);
    int index = hash_pjw(structNode->children[0]->val, tSize);
