...
void saveAll()
{
    ...
    for(int i = 8; i <= 25; ++i)
    {
        if(regsSaved[i].curOp)
        {            
            if(regsSaved[i].curOp->offset != 1)
                codeOff += sprintf(targetCodes+codeOff, "sw $%d, %d($fp)\n", i, regsSaved[i].curOp->offset);                   
            else
            {
                ++count;
                offToFp -= 4;
                codeOff += sprintf(targetCodes+codeOff, "sw $%d, %d($fp)\n", i, offToFp);
                regsSaved[i].off = offToFp;
                regs[i].curOp->offset = offToFp;
            }
            regs[i].curOp->reg = -1;
            regs[i].curOp = NULL;
        }
...
void copyRegsState()
{    
    regsSaved = (struct Register*)malloc(32*sizeof(struct Register));
    //push();
    for(int i = 8; i <= 25; ++i)
    {        
        regsSaved[i].curOp = regs[i].curOp;
        if(regs[i].curOp)
            regsSaved[i].off = 1;
        else
            regsSaved[i].off = 0;
