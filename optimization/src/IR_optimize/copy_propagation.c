...
void CopyPropagation_transferStmt (CopyPropagation *t,
    ...
    if(new_def != IR_VAR_NONE) {
        if(VCALL(fact->def_to_use, exist, new_def)) {
            ...
            for(MapNode_IR_var_IR_var* it = (MapNode_IR_var_IR_var*)TreapNodeBase_first_iter(fact->def_to_use.root);
                it != NULL;)
            {
                TreapNodeBase* tmp = TreapNodeBase_next_iter((TreapNodeBase*)it);
                if(it->val == new_def){
                    MapNode_IR_var_IR_var* it1 = (MapNode_IR_var_IR_var*)tmp;
                    VCALL(fact->def_to_use, delete, it->key);                
                }
                it = (MapNode_IR_var_IR_var*)tmp;
            }
        }
    ...
    for_map(IR_var, IR_var, it, fact->def_to_use)
        printf("def-use: v%d-v%d\n", it->key, it->val);
    printf("\n");
    for_map(IR_var, IR_var, it, fact->use_to_def)
        printf("use-def: v%d-v%d\n", it->key, it->val);
    printf("***************\n");
}
