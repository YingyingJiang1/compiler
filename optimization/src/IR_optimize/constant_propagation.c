...
static CPValue meetValue(CPValue v1, CPValue v2) {
     ...
     */
    // c union c = c, c1 union c2 = NAC
    if(v1.kind == CONST && v2.kind == CONST)
    {
        int v1_const = v1.const_val, v2_const = v2.const_val;
        if(v1_const == v2_const)
            return get_CONST(v1_const);
        else
            return get_NAC();
    }
    // NAC union NAC/UNDEF/CONST = NAC
    if(v1.kind == NAC || v2.kind == NAC)
        return get_NAC();
    // UNDEF union UNDEF = UNDEF
    if(v1.kind == UNDEF && v2.kind == UNDEF)
        return get_UNDEF();
    // UNDEF union c = c
    int const_val = v1.kind == CONST ? v1.const_val : v2.const_val;
    return get_CONST(const_val);
}
